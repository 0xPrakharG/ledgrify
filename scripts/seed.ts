...
const SEED_USER_ID = "user_2mCL0Jp4kOxLUIbxPYmQmRvrdIW";
...
const deafultFrom = subDays(deafultTo, 90);
...
const generateRandomAmount = (category: typeof categories.$inferInsert) => {
    switch (category.name) {
        case "Rent":
            return Math.random() * 400 + 90; // Rent will be a larger amount
        case "Utilities":
...
const generateTransactions = () => {
    const days = eachDayOfInterval({ start: deafultFrom, end: deafultTo });
